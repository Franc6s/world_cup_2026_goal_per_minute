    ...
    [
        ...
        html.Div(
            [
                ...
                card("Top Scorer", value_id="confed-top-scorer"),
            ],
            style={
                "display": "grid",
        ...
        html.Div(
            [
                ...
                html.Div(id="confed-country-table"),
            ],
            style={
        ...
        html.Div(
            [
                graph_card("Goals by Scorer Age", "confed-age-scatter"),
...
def register_callbacks(app):
    ...
    def update_confed(confed):
        ...
        full_name = CONFEDERATIONS_FULL_NAMES.get(confed, confed)
        ...
        if not scorer_df.empty:
            ...
            top_scorer_row = scorer_df.iloc[0]

            first_name = (
                ""
                if pd.isna(top_scorer_row["Player First Name"])
                else str(top_scorer_row["Player First Name"])
            )

            last_name = (
                ""
                if pd.isna(top_scorer_row["Player Name"])
                else str(top_scorer_row["Player Name"])
            )

            top_scorer_name = f"{first_name} {last_name}".strip()

            top_scorer_goals = int(top_scorer_row["Goals"])

            top_scorer_display = (
                f"{top_scorer_name} — {top_scorer_goals} Goals"
            )
        ...
        countries_df = (
            ...
            .agg(
                Goals=("Goal_ID", "count"),
                Goal_Scorers=("Player Name", "nunique"),
            )
            .reset_index()
            .sort_values(
        ...
        age_goals = (
            dff.dropna(subset=["Player Age"])
            .groupby("Player Age")
            .size()
            .reset_index(name="Goals")
            .sort_values("Player Age")
        )
